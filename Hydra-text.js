{
    ...
    // get the Hydra/Strudel object
    window._hydra = getHydra();

    // safe setup of _hydraScope
    if (_hydra?.sandbox?.makeGlobal !== undefined) {
        window._hydraScope = _hydra.sandbox.makeGlobal ? window : _hydra.synth;
    } else {
        // fallback for Strudel or any other Hydra-like object
        window._hydraScope = _hydra || {};
    }
}

// now _hydraScope is safe to use
...
// all the hydraText stuff below stays unchanged
{
    const Source = _hydra?.s?.[0]?.constructor;
    
    if (!Source) {
        console.warn("Cannot find Source constructor; text extension will not work.");
    } else {
        ...
        _hydraScope.text = (str, config) => createOneShot(str, config, true, false, false);
        _hydraScope.strokeText = (str, config) => createOneShot(str, config, false, true, false);
        _hydraScope.fillStrokeText = (str, config) => createOneShot(str, config, true, true, false);
        _hydraScope.strokeFillText = (str, config) => createOneShot(str, config, false, true, true);
